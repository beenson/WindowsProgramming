...
namespace homework1
{
    public partial class CourseSelectingForm : Form
    {
        ...
        public CourseSelectingForm(CourseSelectingFormPresentationModel model)
        {
            ...
            _model.GetModel()._newCourseAdded += UpdateDataGridView;
            _model.GetModel()._newDepartmentAdded += UpdateDataGridView;
            InitializeComponent();

            _model.GetHeaders();

            for (int i = 0; i < _model.GetDepartmentCount(); i++)
            {
                AddTabPage(i);
            }
        }
        ...
        private void AddTabPage(int index)
        {
            ...
            string className = InitialRow(_newDataGridView, index);
            _newDataGridView.Name = className + COURSES;
            // Add To _tabControl
            _tabControl.TabPages.Add(className);
            (_tabControl.TabPages[index]).Controls.Add(_newDataGridView);
            // DataGridView Event
        ...
        public void InitialColumn(DataGridView dataGridView)
        {
            ...
            foreach (string header in headers)
            {
                ...
                dataGridView.Columns[dataGridView.Columns.Count - 1].ReadOnly = true;
            }
        }
        ...
        {
            Department department = _model.GetDepartment(index);
            foreach (Course _course in _model.GetCoursesByDepartment(department).GetCourses())
            {
        ...
        private void UpdateDataGridView()
        {
            ...
            for (int i = _dataGridViews.Count; i < _model.GetDepartmentCount(); i++)
            {
                AddTabPage(i);
            }
