...
namespace homework1
{
    public class CourseSelectingFormPresentationModel
    {
        ...
        // 取得curriculum
        public Curriculum GetCoursesByDepartment(Department department)
        {
            if (!_model.GetDepartments().Contains(department))
                return new Curriculum();
            Curriculum curriculum = new Curriculum(_model.GetAllCoursesByDepartment(department));
        ...
        {
            if (index < 0 || index >= _model.GetNumberOfDepartment())
                return new Department("", "");
            return _model.GetDepartments()[index];
        }
    }
}
