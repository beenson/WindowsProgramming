...
namespace homework1
{
    public class Model
    {
        ...
        public event NewDepartmentAddedEventHandler _newDepartmentAdded;
        public delegate void NewDepartmentAddedEventHandler();
        ...
        public void CourseCrawler(string code)
        {
            ...
            if (_departments.Select(x => x.name).Contains(department.name))
                return;
        ...
        public void AddCourse(Course course)
        {
            if (_courses.Select(x => x.Number).Contains(course.Number) && course.Number != "")
                return;
            AddDepartment(course.Department);
            _courses.Add(course);
        ...
        {
            if (department == null || _departments.Select(x => x.name).Contains(department.name))
                return;
            _departments.Add(department);
            NotifyNewDepartmentAdded();
        }
        ...
        {
            return _departments.Distinct().ToList();
        }
        ...
        // 取的所有課程 by department
        public List<Course> GetAllCoursesByDepartment(Department department)
        {
            return _courses.Where(x => x.Department.Equals(department)).ToList();
        }
        ...
        // 觸發 _newDepartmentAdded
        private void NotifyNewDepartmentAdded()
        {
            if (_newDepartmentAdded != null)
                _newDepartmentAdded();
        }
    }
}