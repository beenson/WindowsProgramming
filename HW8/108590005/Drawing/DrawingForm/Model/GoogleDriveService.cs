...
{
    public class GoogleDriveService : DrawingModel.IGoogleDriveService
    {
        ...
        public void UploadFile(string uploadFileName, string contentType)
        {
            ...
            foreach (var file in ListRootFileAndFolder().Where(a => a.OriginalFilename == uploadFileName))
                DeleteFile(file.Id);
        ...
        {
            var list = ListRootFileAndFolder().Where(a => a.OriginalFilename == fileName).ToList();
            if (list.Count <= 0)
                throw new Exception(DrawingModel.Strings.FILE_NOT_EXIST);
            Google.Apis.Drive.v2.Data.File fileToDownload = list.First();
            const string SPLASH = @"\";
